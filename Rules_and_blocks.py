...
from items import MegaItems, wall, rock, box
...
def new_rule(
    ...
    ActiveRules.append(
        (first_cord, is_cord, finish_cord, (first_name, is_name, finish_name))
    )
...
def checking_for_rule_existence(board):
    ...
    for element in rules:
        ...
        if rules.count(element) >= 2:
            ...
            and board[element[1][1]][element[1][0]]
            and board[element[2][1]][element[2][0]]
        ):
            del ActiveRules[element]
        elif (
            ...
            print(
                board[element[0][1]][element[0][0]][0].name,
                element[-1][0],
                "\n",
                board[element[1][1]][element[1][0]][0].name,
                element[-1][1],
            )
            del ActiveRules[element]
            exec(
                compile(
                    f"globals()['{element[3][0]}'].set_{element[3][2]}(False)",
                    str(),
                    "exec",
                )
            )
        ...
        if isinstance(element[0], ActiveBlocksAction):
            ...
            if x >= 2:  # проверка новых правил по x
                if board[y][x - 1] and board[y][x - 2]:
                    if issubclass(
                        board[y][x - 1][0].__class__, ActiveBlocksIS
                    ) and issubclass(board[y][x - 2][0].__class__, ActiveBlocksObject):
            ...
            if y >= 2:  # проверка новых правил по y
                if board[y - 1][x] and board[y - 2][x]:
                    if issubclass(
                        board[y - 1][x][0].__class__, ActiveBlocksIS
                    ) and issubclass(board[y - 2][x][0].__class__, ActiveBlocksObject):
            ...
            if 1 <= x < 15:  # проверка новых правил по x
                if board[y][x - 1] and board[y][x + 1]:
                    print(
                        issubclass(board[y][x - 1][0].__class__, ActiveBlocksObject),
                        issubclass(board[y][x + 1][0].__class__, ActiveBlocksAction),
                    )
                    if issubclass(
                        board[y][x - 1][0].__class__, ActiveBlocksObject
                    ) and issubclass(board[y][x + 1][0].__class__, ActiveBlocksAction):
                        print(1)
                        ...
                        board[y][x - 1][0].__class__, ActiveBlocksObject
                    ) and issubclass(board[y][x + 1][0].__class__, ActiveBlocksObject):
                        first_name = board[y][x - 1][0].name
                        finish_name = board[y][x + 1][0].name
                        new_rule(
                            first_cord=(x - 1, y),
            ...
            if 1 <= y < 9:  # проверка новых правил по y
                if board[y - 1][x] and board[y + 1][x]:
                    ...
                    if issubclass(
                        board[y - 1][x][0].__class__, ActiveBlocksObject
                    ) and issubclass(board[y + 1][x][0].__class__, ActiveBlocksAction):
                    ...
                    if issubclass(
                        board[y - 1][x][0].__class__, ActiveBlocksObject
                    ) and issubclass(board[y + 1][x][0].__class__, ActiveBlocksObject):
                        finish_name = board[y - 1][x][0].name
                        first_name = board[y + 1][x][0].name
                        new_rule(
                            first_cord=(x, y - 1),
            ...
            if x < 13:  # проверка новых правил по x (блок объект находится слево)
                if board[y][x + 1] and board[y][x + 2]:
                    if issubclass(
                        board[y][x + 1][0].__class__, ActiveBlocksIS
                    ) and issubclass(board[y][x + 2][0].__class__, ActiveBlocksAction):
            ...
            if y < 7:  # проверка новых правил по y (блок объект находится сверху)
                if board[y + 1][x] and board[y + 2][x]:
                    if issubclass(
                        board[y + 1][x][0].__class__, ActiveBlocksIS
                    ) and issubclass(board[y + 2][x][0].__class__, ActiveBlocksAction):
